"""
Bridge between local coding agents and the Wisp clients.
...
class State:
    def __init__(self):
        ...
        self.events_seen = 0
        self.usage = {}
        # Same totals split by provider for the menu-bar panel.  ``usage``
        # remains the combined shape older clients and the board understand.
        ...
        # Codex writes its exact server-provided percentages into local session
        # logs.  The refresh thread fills this without credentials or network.
        self.codex_limits = {}
        self.limits_error = ""
    ...
    def snapshot(self, for_app: bool = False) -> dict:
        """Payload for the board, or the provider-rich variant for the app."""
        now = time.time()
        with self.lock:
        ...
        if lim.get("ok"):
            bars = lim["bars"] if for_app else self._board_limits(lim["bars"])
            snap["lim"] = [
                ...
                {"l": b["label"] if for_app else b.get("board_label", b["label"]),
                 "p": b["pct"], "r": b["resets_in"],
                 "s": b["severity"], "a": b["active"],
                 "x": bool(b.get("expired")),
                 "e": b["elapsed_pct"] if b.get("elapsed_pct") is not None else -1,
                 # Extra keys cost nothing on the Mac and are ignored by an old
                 # firmware.  They let each provider carry its own freshness.
                 "g": b.get("provider", "claude"),
                 "age": b.get("age_s", lim["age_s"]),
                 "src": b.get("source", lim.get("source", ""))}
                for b in bars
            ]
            snap["lim_age"] = max((b.get("age_s", lim["age_s"]) for b in bars),
                                  default=lim["age_s"])
    ...
    @staticmethod
    def _board_limits(bars: list) -> list:
        """Fits both providers into the four physical cards on the board.

        Claude can expose a model-scoped third window and Codex can expose two.
        Five cards do not fit the 480px panel.  Keep both providers' principal
        account windows first; a scoped Claude window uses only a remaining
        slot.  The Mac panel still receives every bar.
        """
        if len(bars) <= 4:
            return bars
        principal = []
        rest = []
        for bar in bars:
            if (bar.get("provider") == "codex"
                    or bar.get("kind") in ("session", "weekly_all")):
                principal.append(bar)
            else:
                rest.append(bar)
        return (principal + rest)[:4]

    def claude_subscription_limits(self) -> dict:
        """
    ...
    def subscription_limits(self) -> dict:
        """All known provider limits, with source and age on every bar."""
        claude = self.claude_subscription_limits()
        with self.lock:
            codex = dict(self.codex_limits)

        groups = []
        if claude.get("ok"):
            groups.append(("claude", claude))
        if codex.get("ok"):
            groups.append(("codex", codex))
        if not groups:
            return {"ok": False, "reason": "no subscription limits available"}

        bars = []
        for provider, group in groups:
            for original in group["bars"]:
                bar = dict(original)
                bar["provider"] = provider
                bar["source"] = group.get("source", "")
                bar["age_s"] = group.get("age_s", -1)
                if provider == "claude":
                    if bar.get("kind") == "session":
                        bar["board_label"] = "Claude 5h"
                    elif bar.get("kind") == "weekly_all":
                        bar["board_label"] = "Claude week"
                    else:
                        model = bar["label"].removeprefix("Weekly ")
                        bar["board_label"] = f"Claude {model}"[:23]
                bars.append(bar)

        sources = {group.get("source", "") for _, group in groups}
        peak = max(bars, key=lambda bar: bar["pct"])
        return {
            "ok": True,
            "bars": bars,
            "age_s": max(group.get("age_s", -1) for _, group in groups),
    ...
    @staticmethod
    def _combined_usage(by_provider: dict) -> dict:
        combined = {
            "input": 0, "output": 0, "cache_read": 0, "cache_write": 0,
            "cost": 0.0, "requests": 0,
        }
        for total in by_provider.values():
            for key in combined:
                combined[key] += total.get(key, 0)
        return combined

    def refresh_usage_once(self) -> None:
        """Recomputes local usage once; split out so the path is testable."""
        claude = usage.collect(since_days=1)
        codex = codex_usage.collect(since_days=1)
        provider_usage = {}
        models = {}
        if "error" not in claude:
            provider_usage["claude"] = claude["total"]
            models.update({f"claude:{key}": value
                           for key, value in claude["by_model"].items()})
        if "error" not in codex:
            provider_usage["codex"] = codex["total"]
            models.update({f"codex:{key}": value
                           for key, value in codex["by_model"].items()})
        if provider_usage:
            combined = self._combined_usage(provider_usage)
            with self.lock:
                self.usage_by_provider = provider_usage
                self.usage = {"total": combined, "by_model": models}
                self.usage_at = time.time()
                if codex.get("limits", {}).get("ok"):
                    self.codex_limits = codex["limits"]
        windows = rolling.calculate()
        if windows.get("ok"):
            with self.lock:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if route == "/state":
            ...
            snap = STATE.snapshot(for_app=True)
            now = time.time()
            with STATE.lock:
                ...
                usage_by_provider = dict(STATE.usage_by_provider)
