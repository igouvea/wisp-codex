...
# How often we recheck the local IP and whether the records are still up.
#
# This is the blind window after a wake: the time the board spends hunting for
# a bridge that is running and unannounced. It costs one UDP route lookup and
# three waitpid()s, so there was nothing being saved by the 30s it used to be.
IP_INTERVAL_S = 10.0
...
def _all_alive() -> bool:
    """
    Every record still published.

    `active()` answers "is anything up", which is the right question for a
    status line and the wrong one here: one surviving process would hide two
    dead ones.
    """
...
def _watch(dns_sd: str, port: int) -> None:
    """
    Keeps the records matching reality.

    TWO THINGS BREAK THEM, AND ONLY ONE WAS COVERED
    -----------------------------------------------
    The address changing was covered. These were not, and both are what a
    laptop does every single day — sleep, leave, come back:

    1. `dns-sd` is a CLIENT of mDNSResponder. When that daemon restarts, which
       macOS does across network transitions, the client's connection breaks
       and the process exits. The records leave with it. The IP never changed,
       so comparing addresses noticed nothing.

    2. Losing the route used to leave `_current_ip` holding the old address.
       Coming back to the SAME address then compared equal and republished
       nothing — and an office DHCP handing back the same lease is the normal
       case, not the exception. Forgetting the address while there is no route
       is what makes the return trip count as a change.

    Together those are "it works until I take it home, and then only a restart
    brings it back".
    """
    global _current_ip
    while not _stop.wait(IP_INTERVAL_S):
        ...
        with _lock:
            if not ip:
                if _current_ip:
                    print(f"[announce] no route — forgetting {_current_ip}")
                    _current_ip = ""
                continue

            alive = _all_alive()
            if ip == _current_ip and alive:
                continue

            if not alive:
                print(f"[announce] records gone (dns-sd exited), republishing on {ip}")
            else:
