...
| Configure fails with `MissingKconfigError: LV_USE_FREETYPE` while resolving LVGL 9.5.0 | older `idf-component-manager` versions can leak missing Kconfig keys while evaluating newer dependency candidates | install the fixed resolver in IDF's venv: `python -m pip install idf-component-manager==2.5.2` |
...
| Serial silent and esptool cannot connect | do not toggle DTR/RTS to force a reset — on native USB-JTAG those lines drive reset and boot mode, and it leaves the port mute | unplug and replug the USB-C cable. **This board has no reset button** — but if a battery is fitted, the cable is NOT the reset either: unplugging USB only re-enumerates the USB device, the MCU keeps running on the cell. To actually restart it you need `esptool ... --after hard_reset`, or the firmware's own reboot |
...
| Wisp asks for the `login` keychain password over and over — ten times a day — even with a stable signing identity | macOS checks two lists, and only one of them survives. The item's ACL keeps the app (the designated requirement is stable since `create-identity.sh`); the **partition list** does not. Claude Code saves the credential with `security add-generic-password -U`, and an update through that tool rewrites the partition list as `apple-tool:`, dropping the entry your password just added — once per token refresh, and once per OAuth grant of every MCP server, since they share this item. The tell: the ACL had five Wisp entries, one per build, and the partition list exactly one | read it through `/usr/bin/security`, the one caller both lists already trust, and keep the token until its own `expiresAt` — `mac/Sources/Limits.swift` does both. The direct `SecItemCopyMatching` stays as the fallback |
...
path is verified on hardware, and since 2026-08-25 **the S3 path is verified on
hardware too**: the bench S3 (identified by PSRAM being present at all)
ran an `idf.py app-flash` of a `main.c` change and was confirmed talking to the
bridge again. Note the S3 carries a battery, so see the reset row above before
assuming a power-cycle happened.
