...
Pin the component manager used by IDF 5.5. Older resolver releases can fail
before compilation with a false `MissingKconfigError: LV_USE_FREETYPE` while
examining LVGL dependency candidates:

```bash
python -m pip install idf-component-manager==2.5.2
```
