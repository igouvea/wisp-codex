...
void ui_update(const wisp_data_t *d)
{
    ...
    for (int i = 0; i < d->limit_count && i < MAX_BARRAS; i++) {
        ...
        assinatura = assinatura * 31u + (uint32_t) b->expired;
        assinatura = assinatura * 31u + (uint32_t)(b->elapsed_pct + 1);
    ...
    if (primeiro_update || assinatura != ult_assinatura) {
        ...
        for (int i = 0; i < MAX_BARRAS; i++) {
            ...
            char tmp[40];
            if (b->expired) snprintf(tmp, sizeof(tmp), "--");
            else snprintf(tmp, sizeof(tmp), "%d%%", b->pct);
            ...
            lv_bar_set_value(g_bar[i], b->expired ? 0 : b->pct, LV_ANIM_OFF);
            lv_obj_set_style_bg_color(g_bar[i], cor_do_pct(b->pct), LV_PART_INDICATOR);
            posicionar_ritmo(g_bar_ritmo[i], g_bar_marca[i],
                             b->expired ? -1 : b->elapsed_pct);

            if (b->expired) {
                lv_label_set_text(g_bar_reset[i], "expired");
            } else if (b->resets_in[0]) {
