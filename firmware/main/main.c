...
{
    /* Onde comecar a olhar quando vem mais de uma resposta.
     *
     * O Mac publica TRES instancias de `_wisp._tcp`: o servico de verdade e
     * dois proxies de nome (`wisp.local` e o hostname legado). Pegar sempre a
     * primeira com endereco parece inofensivo e nao e: se ela trouxer um
     * endereco que nao serve mais, insistir nela a cada rodada nunca chega nas
     * outras, e a placa fica presa numa resposta ruim tendo a boa na mesma
     * lista. Girar o ponto de partida custa um inteiro e faz com que toda
     * resposta seja experimentada. Invisivel quando tudo esta bem: com o
     * bridge no ar isto roda uma vez e nao volta. */
    static unsigned rodada = 0;
    ...
    if (mdns_query_ptr("_wisp", "_tcp", 3000, 4, &r) == ESP_OK && r) {
        ...
        if (n > 0) {
            int alvo = (int) (rodada++ % (unsigned) n), i = 0;
            for (mdns_result_t *it = r; it; it = it->next) {
                if (!it->addr) continue;
                if (i++ != alvo) continue;
                esp_ip4_addr_t a = it->addr->addr.u_addr.ip4;
                snprintf(s_ip, sizeof(s_ip), IPSTR, IP2STR(&a));
                ESP_LOGI(TAG, "bridge achado pelo servico mDNS -> %s (%d de %d)",
                         s_ip, alvo + 1, n);
...
static bool interpretar(const char *json, wisp_data_t *d)
{
    ...
    cJSON_ArrayForEach(it, lim) {
        ...
        b->expired = cJSON_IsTrue(cJSON_GetObjectItemCaseSensitive(it, "x"));
...
static void tarefa_rede(void *arg)
{
    ...
    while (1) {
        ...
        if (r == ESP_OK && status == 200 && c.usado > 0) {
             ...
             *
             * O `>=` e o zerar de `falhas` consertam uma armadilha que só
             * aparece depois de uma viagem — levar o Mac para casa e voltar.
             * Com `== 5` este ramo disparava UMA vez na vida da task: se a
             * resolução seguinte devolvesse um endereço que não serve mais (o
             * Mac voltou noutro IP do DHCP, ou o anúncio velho ainda estava no
             * ar quando a placa perguntou), `falhas` seguia para 6, 7, 8… e
             * nunca mais batia em 5. O `s_ip` ruim ficava soldado, nenhuma
             * resolução nova era tentada, e `ui_update` nunca mais era chamado:
             * a tela congelava no último texto pintado — "bridge not found" —
             * e só voltava com reboot.
             *
             * MEDIDO assim: bridge de pé há 19h, placa respondendo ping na
             * mesma rede, mDNS do Mac anunciando certo há 20 minutos, e o
             * bridge sem receber UMA consulta da placa em 16,7 horas. */
            ESP_LOGW(TAG, "bridge inacessível em %s (%s, status %d) — resolvendo de novo",
                     s_ip, esp_err_to_name(r), status);
            wisp_data_t off = {.session_count = 1, .limits_age_s = -1};
            off.sessions[0].state = WISP_OFFLINE;
            /* O ENDEREÇO na tela, não só "offline".
             *
             * É o que separa "o Mac não está na rede" de "a placa está
             * teimando num IP que não é mais dele" — as duas ficavam iguais
             * aqui, e essa é a diferença entre esperar e reiniciar. Sem serial
             * ligado, a tela é o único log que existe. */
