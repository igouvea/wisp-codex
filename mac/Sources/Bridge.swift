...
final class Bridge: ObservableObject {
    ...
    private func updateLimits() async {
        ...
        do {
            ...
            // A rejected token is a token we should stop reusing. The cache in
            // Limits is keyed on the credential's own expiry, and a re-login
            // in Claude Code rotates the token long before that date arrives.
            if case .http(let code, _) = f, code == 401 || code == 403 {
                Limits.invalidate()
            }
