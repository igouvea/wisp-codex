...
struct Panel: View {
    ...
    private func limitGroup(_ d: AppState, provider: String) -> some View {
        let providerLimits = d.limits(for: provider)
        let age = d.limitAge(for: provider)
        let source = d.limitSource(for: provider)
        let trustworthy = providerLimits.allSatisfy { d.limitTrustworthy($0) }
        return VStack(alignment: .leading, spacing: 5) {
            HStack {
                ...
                if age >= 0 {
                    ...
                    // an API reading, "cache" is Claude Code's disk copy, and
                    // Codex "local" comes from its newest transcript event.
                    Text("\(source.isEmpty ? "unknown" : source) · \(shortAge(age))")
                        .font(.system(size: 9, weight: .medium))
                        .foregroundStyle(trustworthy ? Color.secondary.opacity(0.7)
                                                    : Palette.severity("warning"))
                }
            }
            ForEach(providerLimits) {
                LimitBar(limit: $0, trustworthy: d.limitTrustworthy($0))
            }
        }
    }
    ...
    private func windowUsage(_ w: Windows) -> some View {
        VStack(alignment: .leading, spacing: 7) {
            SectionHeader(title: "Claude · local usage")
    ...
    private func todayUsage(_ d: AppState) -> some View {
        VStack(alignment: .leading, spacing: 5) {
            ...
            ForEach(d.usageProviders, id: \.self) { provider in
                usageRow(d.providerLabel(provider), d.usage(for: provider))
            }
        }
    }

    private func usageRow(_ provider: String, _ usage: Usage) -> some View {
        HStack(spacing: 7) {
            Text(provider)
                .font(.system(size: 11, weight: .semibold))
                .frame(width: 46, alignment: .leading)
            Text("\(usage.requests ?? 0) req")
            Spacer(minLength: 2)
            Text("\(compact(usage.output)) out")
            Text("\(compact(usage.cache_read)) cache")
        }
        .font(.system(size: 10).monospacedDigit())
        .foregroundStyle(.secondary)
    }
