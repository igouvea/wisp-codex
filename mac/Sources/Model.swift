...
struct Limit: Decodable, Identifiable {
    ...
    let x: Bool?
    /// Provider and freshness live on each bar because Claude and Codex are
    /// read from different sources at different instants. Optional keeps a new
    /// app compatible with an older bridge.
    let g: String?
    let age: Int?
    let src: String?

    var id: String { "\(provider)|\(l)" }
    var expired: Bool { x == true }
    var provider: String { g ?? "claude" }
...
struct AppState: Decodable {
    ...
    let usage: Usage
    /// Today's local transcript totals, kept separate so adding Codex does not
    /// turn a combined number into an attribution lie. Optional for old bridges.
    let usage_by_provider: [String: Usage]?
    ...
    var limitProviders: [String] {
        let available = Set(limits.map(\.provider))
        return ["claude", "codex"].filter { available.contains($0) }
            + available.filter { $0 != "claude" && $0 != "codex" }.sorted()
    }

    func limits(for provider: String) -> [Limit] {
        limits.filter { $0.provider == provider }
    }

    func limitAge(for provider: String) -> Int {
        limits(for: provider).compactMap(\.age).max() ?? limits_age_s
    }

    func limitSource(for provider: String) -> String {
    ...
    func limitTrustworthy(_ limit: Limit) -> Bool {
        let valueAge = limit.age ?? limits_age_s
        let source = limit.src ?? limits_source ?? ""
        return valueAge >= 0 && valueAge < (source == "live" ? 1800 : 300)
    }

    var usageProviders: [String] {
        guard let split = usage_by_provider, !split.isEmpty else { return ["total"] }
        let available = Set(split.keys)
        return ["claude", "codex"].filter { available.contains($0) }
            + available.filter { $0 != "claude" && $0 != "codex" }.sorted()
    }

    func usage(for provider: String) -> Usage {
        if provider == "total" { return usage }
        return usage_by_provider?[provider] ?? usage
    }
